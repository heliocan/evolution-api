...
```shell
sudo nano /etc/apache2/sites-available/evo.dominio.com.conf
```

Conteúdo:
```shell
<VirtualHost *:80>
    ServerName evo.dominio.com
    ...
    RewriteEngine on
    RewriteCond %{SERVER_NAME} =evo.dominio.com
...
```bash
sudo a2ensite evo.dominio.com.conf
```
